...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int = 2):
        ...
        self.training_args = OptimizationParams_3DGS()
    ...
    def get_scaling(self):
        #return self.scaling_activation(self._scaling[:,:1].repeat(1,3))
    ...
    def create_from_pcd(self, points, spatial_lr_scale : float ):
        ...
        scales = torch.full((N, 3), -4.5, device="cuda")
        rots = torch.zeros(N, 4).cuda()
        rots[:, 0] = 1
    ...
    def save_pointcloud(self,scale):
        points = self.get_xyz.detach().cpu().numpy()/scale

...
__forceinline__ __device__ float NonSquareNdcRange(int S1, int S2) {
  float range = 2.0f;
  if (S1 > S2) {
...
__forceinline__ __device__ float PixToNonSquareNdc(int i, int S1, int S2) {
...
__forceinline__ __device__ int NonSquareNdcToPix(float ndc, int S1, int S2) {
  float range = NonSquareNdcRange(S1, S2);
  float offset = (range / 2.0f);
  // Reverse the NDC calculation:
  // (range * i + offset) / S1 = ndc + offset
  // range * i + offset = (ndc + offset) * S1
  // range * i = (ndc + offset) * S1 - offset
  // i = ((ndc + offset) * S1 - offset) / range
  float pixel = ((ndc + offset) * S1 - offset) / range;
  // Round to nearest integer and clamp to valid range
  return max(0, min(S1 - 1, int(pixel + 0.5f)));
}

__forceinline__ __device__ float ndc2Pix(float v, int S1, int S2)
{
	//return NonSquareNdcToPix(v,S1,S2);
	return ((v + 1.0) * S1 - 1.0) * 0.5;
}
...
__forceinline__ __device__ bool in_frustum2(int idx,
	const float* orig_points,
	const float* viewmatrix,
	const float* projmatrix,
	bool prefiltered,
